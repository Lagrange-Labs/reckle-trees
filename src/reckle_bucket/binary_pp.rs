...
use crate::reckle_bucket::binary_circuits::{
...
type CurrentGateSerialize = DefaultGateSerializer;
    ...
    [(); 1 << Q]:,
{
    ...
    pub leaf_wire_information: LeafWireInformation<Q, D>,
    ...
    [(); 1 << Q]:,
{
    pub fn setup(log_max_capacity: u8) -> Self {
        ...
        let (leaf_wire_information, common_leaf, prover_leaf, verifier_leaf) =
            leaf_circuit_setup::<Q, F, C, D>(None);
        ...
        for _ in (Q as u8) + 1..=log_max_capacity {
            let (wire_information, common, prover, verifier) =
                internal_circuit_setup::<F, C, C, D>(&prev_common, prev_hash, None);
    ...
    pub fn size_by_level(&self) {
        ...
        let gate_serializer = CurrentGateSerialize {};
        let gen_serializer = DefaultGeneratorSerializer::<C, D> {
        ...
        println!(
            ...
            Q, self.log_max_capacity, common_leaf_size, prover_leaf_size, verifier_leaf_size
        );
